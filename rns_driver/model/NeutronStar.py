...
class NeutronStarEOSCollection:
    ...
    def filter_by_central_density(
        self, min_rho=-1.0, max_rho=-1.0, last_entry=False
    ) -> pd.DataFrame:
    ...
    def delete_outside_bounds(
        ...
        if min_rho < 0 and max_rho < 0:
            print(
                "Why did you call this filter function if you do not filter? You deserved being filtered out of mankind!"
            )
            return
        ...
        while True:
            ...
            if previous_rho_c is not None:
                if invalid_star_counter == 0:
                    incline = (star.M - previous_M) / (
                        rho_c - previous_rho_c
                    )  # Change in M divided by change in rho_c
    ...
    def traverse_r_ratio(
        self, rho_TOV, initial_stepsize_ratio, initial_stepsize_rho_c=1e13
    ):
        ...
        while True:
            ...
            if lenght_of_dataframe_previously != 0:
                ...
                if (
                    filtered_df.loc[lenght_of_dataframe_previously - 1].at["Omega"]
                    > omega_kepler
                ):
                    invalid_collection_counter += 1
                    stepsize_ratio = stepsize_ratio / 2.0
                    ratio = previous_ratio + stepsize_ratio
                    print(
                        "Let's see if this makes sense. I will have to check my angular momentum"
                    )

            print("lenght_of_dataframe_previously",
                  lenght_of_dataframe_previously)
            self.get_series(rho_TOV, initial_stepsize_rho_c,
                            {"r_ratio": ratio})
...
class NeutronStarEOSCatalog:
    ...
    def add_eos_collection(self, eos_name, eos_collection):
        ...
        if eos_name in self.eos_collections:
            print(
                f"Warning: EOS named {eos_name} already exists. Overwriting.")
    ...
    def _process_single_eos(self, eos_path):
        ...
        eos_collection = NeutronStarEOSCollection(
            eos_path
        )  # instantiate your EOSCollection here
